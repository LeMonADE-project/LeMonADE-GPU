...
void trackBreaks( ID_t * const dBreaks, uint32_t const size, ID_t * const miNewToi, uint32_t const mAge);
void trackConnections( ID_t * const mCrossLinkFlags, ID_t * const mCrossLinkIDS,
		       ID_t   const size, ID_t * const miNewToi, uint32_t const offset, uint32_t const mAge);
//! setter function for the buffer size
void setBufferSize(uint32_t bufferSize_){bufferSize=bufferSize_;}
MirroredVector<ID_t> * BondHistory;
